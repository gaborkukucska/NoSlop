...
## ADR-009 — Tor layer is reached through a SOCKS5 proxy seam; daemon/onion are separate, platform-specific steps
- **Status:** Accepted (2026-06-17)
- **Context:** Phase 1 step 8 has a real iOS leaf gossiping to a real desktop HUB over **plain TCP**. The privacy story (ADR-002) needs that link to run over Tor: the HUB exposes a v3 **onion hidden service**; the leaf dials it through Tor's **SOCKS5 proxy** (Tor resolves the `.onion` and tunnels). The Android app already does exactly this (`Proxy(SOCKS, 127.0.0.1:9050)` + `TorService` hidden service). Three sub-problems have very different cost/risk: the SOCKS5 *client* is pure protocol code; running a Tor *daemon* on desktop needs a bundled `tor` binary; Tor on *iOS* needs a native framework (Tor.framework/iCepa, or an Arti/Rust path).
- **Decision:** Decompose Tor into three independently-shippable pieces, behind the existing `Transport` seam:
  1. **SOCKS5 client dialing** *(buildable + unit-testable now, no Tor needed).* Add optional SOCKS5 (RFC 1928, CONNECT, domain ATYP for `.onion`) support to `SocketTransport.connect(host, port, proxy)`. Test against a mock SOCKS5 server in-process. This is the mechanism every later piece relies on; it's the part we build first.
  2. **Desktop HUB Tor daemon + onion registration** *(needs a bundled/installed `tor` binary + control-port wiring).* The HUB launches/locates Tor, registers an ephemeral v3 hidden service (ADD_ONION over the control port) pointing at its `SocketTransport.listen()` port, and prints the onion address. **Owner decision (2026-06-17): BUNDLE `tor` with the HUB** (ship per-OS binaries — zero user friction vs. requiring a system install). The control-port client (`TorControl.addOnion`) is built + tested (step 9c); remaining = vendor + spawn the binary, then verify a live onion.
  3. **iOS Tor.** iOS can't run `tor` as a plain binary. **Owner decision (2026-06-17): use Tor.framework / iCepa** (battle-tested CocoaPods/SwiftPM; fastest path to a working onion — Arti/Rust revisitable later). Integration = start Tor on launch, read its SOCKS port, feed `SocksProxy(127.0.0.1, torSocksPort)` + the `.onion` to `MeshClient.connect` (dialing side already done, steps 9a/9b). Done interactively (Xcode/CocoaPods + device + Tor bootstrap), not unattended.
- **Consequences:** The SOCKS5 client lands and is tested now, so the leaf can already dial *any* SOCKS5 proxy (incl. a system Tor) — proving the path end-to-end against a mock proxy. Plain-TCP and SOCKS/Tor coexist (the `proxy` arg is optional), so loopback/LAN demos keep working while Tor is added. Pieces 2 and 3 are gated on external dependencies and are flagged for the owner rather than guessed at.
