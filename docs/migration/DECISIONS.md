...
- **Decision:** Decompose Tor into three independently-shippable pieces, behind the existing `Transport` seam:
  ...
  2. **Desktop HUB Tor daemon + onion registration** *(needs a bundled/installed `tor` binary + control-port wiring).* The HUB launches/locates Tor, registers an ephemeral v3 hidden service (ADD_ONION over the control port) pointing at its `SocketTransport.listen()` port, and prints the onion address. Environment-dependent (requires Tor present), so it is a deliberate, separately-verified step — not attempted blind.
  3. **iOS Tor** *(needs a native dependency decision).* iOS can't run `tor` as a plain binary; options are Tor.framework (CocoaPods/SwiftPM) or a future Arti (Rust) embedding. This is a dependency/architecture decision for the owner and the largest unknown; scoped, not built, until chosen.
