...
153. **Mesh Comment Synchronization Fix**: Modified `SyncResponsePayload` to embed and propagate arrays of historical `COMMENT` and `REACTION` entities alongside posts, fixing the siloed engagement bug where peers only saw their own interactions.
154. **gChat UI Parity for Reactions**: Replaced the long-press reaction interaction with a single-tap trigger matching the `gChat` reference. Refactored active reactions into inline, tappable pill counters (e.g. `❤️ 3`) embedded directly into the feed card action row.
155. **Background Service Persistence**: Built and wired `NoSlopForegroundService.kt` to bind the Tor daemon (`TorService`) to an active Foreground Service notification. This prevents Android 8+ OS limitations from silently killing background mesh synchronisation.
156. **Online Presence System (`ANNOUNCE_PEER`)**: Implemented gChat's "Live Contact Sync". `NoSlopRepository` now broadcasts a signed heartbeat packet every 60 seconds. `MeshPacketHandler` verifies and processes these to update `Peer.isOnline` and `lastSeenAt` in the Room database, while stale peers automatically time out after 3 minutes. `PeerItem.kt` renders a real-time green online indicator.
157. **Documentation Alignment**: Corrected legacy discrepancies in `GAP_ANALYSIS.md`, `README.md`, and `CryptoService.kt` regarding X25519/ChaCha20-Poly1305 DM cryptography and `clearnet_thumbnail_url` propagation.
158. **Relay State Memory Management**: Added `establishedAt` and `lastActivity` tracking to `GossipService.RelayState`, plus a periodic coroutine sweeper that evicts stale relay routes after 5 minutes of inactivity. `MeshPacketHandler` now refreshes `lastActivity` on every `MEDIA_CHUNK` via `GossipService.touchRelayState()`, preventing premature eviction of active transfers.
159. **Hash-Based Inventory Sync (`INVENTORY_SYNC_REQUEST`)**: Replaced the inefficient timestamp-based `SYNC_REQUEST` with a hash-diffing protocol. On handshake, the node sends its local post inventory as `{id, SHA3-256 hash}` pairs. The receiver compares hashes and only returns genuinely missing or divergent posts in the `SYNC_RESPONSE`, drastically reducing redundant transfer over Tor.
160. **Avatar Persistence & Custom Cropping**: Replaced external avatar URL loading with local persistent image cropping. Users can now pick an image, crop it to a 1:1 ratio with custom scale/pan, and save it as a high-compression JPEG Base64 string within their EncryptedSharedPreferences identity.
161. **Persistent Notification Architecture**: Built a local Room-backed notification system (`NotificationItem` / `NotificationDao`) that logs inbound DMs and comments. These power a dedicated in-app history dashboard, replacing transient system-only push alerts.
162. **Dynamic UI Badging & Deep Linking**: Integrated Material 3 `BadgedBox` indicators across the bottom navigation and feed screens. Notifications now carry a `targetRoute` payload, enabling seamless deep-linking back to specific chat threads or mesh posts when interacted with.
163. **Handshake Signature Hardening**: Enhanced `CONNECTION_REQUEST` and `USER_HANDSHAKE` payloads to include and cryptographically sign the `authorAvatarB64` field. This prevents man-in-the-middle node operators from spoofing or altering avatars during initial peer discovery and connection negotiations.
160. **AIMD Congestion Control for Media Downloads**: Replaced the flat `MAX_CONCURRENCY = 4` / `DOWNLOAD_TIMEOUT_MS = 60000` with an Additive-Increase/Multiplicative-Decrease (AIMD) controller per `ActiveDownload`. Window starts at 4, caps at 128, grows additively on ACK (slow-start then congestion avoidance), and halves on timeout (Reno-style). Timeout adapts dynamically via an Exponential Moving Average of chunk RTT (`2x RTT_EMA`, min 2s).
161. **SHA3-256 Provider Fix**: Fixed a `NoSuchAlgorithmException: SHA3-256 MessageDigest not available` crash on Android devices. Android's default `Conscrypt` provider does not expose SHA3-256 via `java.security.MessageDigest`. Replaced all `MessageDigest.getInstance("SHA3-256")` calls with BouncyCastle's native `SHA3Digest(256)`, matching the approach used throughout `CryptoService.kt`.
162. **Extended Social Reactions**: Added `CHAT_REACTION` and `COMMENT_REACTION` capabilities. Users can now long-press any direct message or mesh comment to open a gChat-style emoji reaction picker. Reactions are securely signed, synced over the mesh, and natively persisted.
163. **Identity Change Propagation (`IDENTITY_UPDATE`)**: Wired up the profile settings so that updating a local handle/display name instantly triggers an `IDENTITY_UPDATE` packet over the network, ensuring peers have up-to-date profile metadata without requiring a full re-handshake.
164. **Graceful Shutdown Protocol (`USER_EXIT`)**: Added an automated `USER_EXIT` packet broadcast hook inside `NoSlopForegroundService.onDestroy()` and explicit app logout. This ensures peers immediately set their local `isOnline` flag to false when the user closes the app, eliminating the 3-minute timeout window for stale peers.
165. **Identity Dual-Write Recovery**: Fixed identity loss during destructive Room migrations by implementing a dual-write strategy. Hardware-backed `EncryptedSharedPreferences` now acts as the immutable source of truth, automatically re-seeding Room on startup if the database is reset.
166. **Tor Timeout Resiliency**: Increased `connectTimeout` and `readTimeout` to 45 seconds in `TorService.kt` to prevent false-positive circuit failures on high-latency mobile networks.
167. **Full-Screen Keyboard UI Adaptability**: Applied `Modifier.imePadding()` with structural scaffolding tweaks to `ChatThreadScreen` and `CommentsBottomSheet` to dynamically resize content above the software keyboard, eliminating obscured inputs.
168. **Clearnet Native Embedding**: Replaced the disjointed "ClearnetAttachment" overlay with native inline rendering. Shared YouTube, Vimeo, and Article links now load seamlessly via `VideoPlayer` and `SegmentedArticleReader` directly inside the primary mesh `FullScreenMeshCard`.
169. **System Push Notifications**: Integrated `NotificationHelper.kt` to trigger native Android heads-up alerts for direct messages and mesh post comments, hooking directly into `MeshPacketHandler` dispatch streams.
170. **Background WebView Media Playback**: Implemented custom `onWindowVisibilityChanged` interception for `WebView`-backed `VideoPlayer` instances, tricking the Chromium renderer into persisting YouTube/Vimeo playback while the app is backgrounded.
171. **Tor Service Recovery**: Refactored `TorService.kt` to force an explicit state transition to `FAILED` if a connectivity check fails while the service is in the `READY` state, ensuring the daemon auto-heals without user intervention.
172. **Network-Synchronized DM Timestamps**: Upgraded `EncryptedPayload` with a sender-injected `timestamp` field. Receivers now sort incoming messages based on the sender's origin time rather than local receive time, eliminating chronological desync during mesh latency spikes. Coupled with auto-scroll-to-bottom logic in `ChatThreadScreen` when new messages arrive.
173. **Scroll Position Persistence**: Restructured `UnifiedFeedTab.kt` to use visibility toggling (`Modifier.alpha(0f)`) rather than destructive `when` compositional switching. `VerticalPager` state and exact scroll positions are now perfectly preserved when navigating between tabs.
179. **Instant Broadcast Injection**: Overhauled `composeAndBroadcastPost` return structures so newly authored mesh posts are instantly prepended to the user's `UnifiedFeed` and auto-scrolled to the top, providing immediate visual feedback without waiting for background sync cycles.
180. **"Welcome Back" Splash Sequence**: Integrated a 3-second animated splash screen (`SplashScreen.kt`) that triggers on subsequent app startups to mask initial content loading. Intelligently tracks `didJustFinishOnboarding` state to skip the animation directly after a new user completes the initial setup flow.
181. **Dynamic UI Icon Refactoring**: Overhauled primary navigation layouts. Moved the Notifications icon to the top-left of the main Feed page (alongside the refresh indicator) for aesthetic balance. Implemented dynamic bottom-navigation mapping that automatically renders a dedicated "Alerts" `NavigationBarItem` in the center slot whenever the user navigates away from the Feed tab.
182. **Frictionless Onboarding**: Updated the `OnboardingScreen` to aggressively pre-select all relevant sources based on the user's genre/category choices, flipping the flow to a subtractive "opt-out" model rather than an additive one. Pre-loads the feed in the background during the final "QR Scan" step to guarantee a populated wall upon entry.
183. **Hardcoded Source Decommissioning**: Purged static, hardcoded YouTube RSS channel presets from `SourceLibrary.kt`. The "Video Platforms" category is now entirely driven dynamically via `InvidiousApiClient`, `VimeoApiClient`, and `InternetArchiveClient` based on user search and genre preferences.
184. **Custom Search Universality Fix**: Repaired a logic bug in `NoSlopRepository.searchCustomFeed` where explicit online searches were inappropriately constrained by the user's active API subscriptions. Search now universally spans all built-in API providers regardless of live-feed toggle states, guaranteeing comprehensive results.
185. **Search Keyboard Integration**: Wired native `ImeAction.Search` and `keyboardActions` handlers into the custom `UnifiedFeedTab` search modal. Users can now successfully execute queries by tapping "Enter" on their device's software keyboard, fixing a frustrating interaction dead-end.
186. **Permanent Viewed History**: Added `viewed_history` Room table to track all content viewed for >5 seconds. History is permanent and capped at 5000 items (oldest-first pruning) to maintain DB performance. The History filter now correctly pulls directly from this table.
187. **Swipe-Away Exclusion**: Implemented double-swipe tracking (`swipe_tracker` table). Items quickly swiped past (<5s dwell time) twice are permanently excluded from the feed, but are *not* removed from history if they were previously viewed.
188. **Privacy Filtering**: Self-authored mesh broadcasts are now automatically excluded from the standard aggregated "Live Feed" and category feeds. They remain accessible under the specific "Mesh" filter.
189. **Universal Search Bypass**: Configured the ViewModel such that any active keyword search instantly bypasses all feed exclusions (self-broadcasts, history, and swipe tracking) to ensure comprehensive search results.
190. **Engagement Accuracy Fix**: Repaired the "Liked" feed list. Items are now only auto-saved to this list upon explicit positive interactions (liking or sharing to mesh).
191. **Database Schema Increment (v23)**: Bumped Room database schema to version 23 to accommodate new tracking entities, relying on `fallbackToDestructiveMigration()` for state reset.
192. **Feed State Consistency**: Re-engineered the manual refresh pipeline (`refreshFeeds()`) to abandon aggressive cache clearing. Manual pulls and background database syncs now smoothly append newly fetched data to the bottom of the feed instead of violently teleporting the user to the top.
193. **Stable Pager Indices**: Removed the eager in-memory deletion of swiped-away items. Items swiped (<5s) are recorded in the DB to exclude from future feed generation, but are retained in the current scroll session's memory to prevent the `VerticalPager` indices from collapsing and pulling unseen items into view unexpectedly.
194. **Chronological Anchor Lock**: Implemented an "Anchor Time" mechanism within `loadMoreFeedItems()`. The feed establishes its chronological anchor via the top-most item's timestamp. Background syncs that discover genuinely new items (newer than the anchor) are held in reserve until the next manual refresh, preventing jarring chronological disruptions during active scroll sessions. Late-arriving older items (e.g., slow videos) correctly bypass this lock and append seamlessly to the bottom.

195. **Media Request Sender ID Fix**: Fixed a critical bug in `MediaManager.requestNextChunks()` where outgoing `MEDIA_REQUEST` packets carried `senderId = repo.peerDao.hashCode().toString()` — a meaningless JVM object hashcode rather than a real public key. Since the receiving peer replies using `packet.senderId` as the destination onion address, every chunk request sent through the AIMD pipeline (milestone 160) went unanswered and the download window collapsed to 1 indefinitely. Packet construction now happens inside the existing coroutine using the local node's real `publicKeyB64` via `repo.getLocalIdentity()`.

196. **Signed USER_EXIT & Non-Blocking Shutdown Broadcast**: Hardened the graceful shutdown protocol (milestone 164). `UserExitPayload` now carries a `signature` field; `broadcastUserExit()` signs `userId|timestamp` with the local Ed25519 key, and `handleUserExit` verifies the signature and rejects packets where `senderId != userId`, preventing a trusted peer from forging another peer's exit. Additionally, `NoSlopForegroundService.onDestroy()` no longer calls `runBlocking` on the broadcast (ANR risk on slow Tor circuits) — it now uses a new fire-and-forget `broadcastUserExitAsync()` bounded by a 3-second `withTimeoutOrNull`, falling back to the existing 3-minute `ANNOUNCE_PEER` staleness timeout for unreached peers.
...
- ~~**Comment Module UI**~~: ✅ Resolved in milestone 167. Input field now stays visible above the keyboard.
- ~~**Handshake Signature Verification Gap**~~: ✅ Resolved (milestone 163). `handleConnectionRequest` and `handleUserHandshake` in `MeshPacketHandler.kt` now reconstruct `fromUserId|fromUsername|fromHomeNode|timestamp` (plus `|authorAvatarB64` when present) and verify it via `CryptoService.verify` before accepting the peer, rejecting on a missing or invalid signature.
- ~~**Relay State Memory Growth**~~: ✅ Resolved in milestone 158. `GossipService.relayStates` now has `establishedAt`/`lastActivity` tracking and a periodic 60-second sweep that evicts routes idle for >5 minutes.
- **Notification Deep Linking**: Tapping on push notifications currently only opens the app to the main screen. It does not yet navigate the user directly to the specific DM thread or mesh post comment.
...
1. ~~**Enhanced Reactions UI**~~: ✅ Resolved (milestone 90, 154, 162). `REACTION`, `CHAT_REACTION` and `COMMENT_REACTION` packets, signed/gossiped reaction toggling, and the soft-block/hard-block Content Health overlay are implemented. The UI has been updated to match gChat's standard, using a single-tap trigger to open the reaction picker, and displaying active reactions as inline, tappable pill counters (e.g. `❤️ 3`).
2. **Group Chats**: Implement decentralized group chat capabilities with dedicated mesh packet routing. gChat already has a complete reference spec (`Group` entity, `GROUP_INVITE`/`GROUP_UPDATE`/`GROUP_DELETE`/`GROUP_QUERY`/`GROUP_SYNC` packets, multi-recipient X25519 encryption fan-out) — see [GAP_ANALYSIS.md](GAP_ANALYSIS.md) §3 for the full spec to port.
3. ~~**Mesh Comment Synchronization Fix**~~: ✅ Resolved. Refactored the `SyncResponsePayload` to include `comments` and `reactions` arrays, ensuring full mesh propagation of historical engagement beyond just raw posts. Users now see comments and reactions from other peers globally.
4. ~~**Sophisticated Notification System**~~: ✅ Resolved in milestone 169. Implemented Android local notifications (`NotificationHelper.kt`) triggering heads-up alerts for direct messages and mesh post comments directly off the `MeshPacketHandler` streams. *Note: Deep linking is still pending; tapping notifications currently only opens the app, rather than navigating to the specific thread.*
5. ~~**Semi-Active Background Mode (Foreground Service)**~~: ✅ Largely resolved. Built `NoSlopForegroundService.kt` to bind the Tor daemon and keep the mesh connection alive while the phone is locked, preventing Android background execution limits. Remaining: auto-lock timeouts and user toggles for battery savings.
...
See [docs/GAP_ANALYSIS.md](GAP_ANALYSIS.md) for further parity items versus gChat/HAI-Net (presence/`ANNOUNCE_PEER`, `COMMENT_REACTION`/`COMMENT_VOTE`, hash-based `INVENTORY_SYNC`, group chats, etc.).
...
## Further Reading
- [docs/TECHNICAL_REFERENCE.md](TECHNICAL_REFERENCE.md) — exhaustive technical reference covering package layout, exact crypto derivation formulas, gossip pipeline internals, the full wire protocol, media/Tor internals, Room schema, and build configuration.
- [docs/GAP_ANALYSIS.md](GAP_ANALYSIS.md) — detailed comparison against gChat and HAI-Net's `hainet-social` Rust crate, with a prioritized backlog checklist for packet types, presence, group chats, congestion control, and more.