...
174. **Instant Broadcast Injection**: Overhauled `composeAndBroadcastPost` return structures so newly authored mesh posts are instantly prepended to the user's `UnifiedFeed` and auto-scrolled to the top, providing immediate visual feedback without waiting for background sync cycles.
