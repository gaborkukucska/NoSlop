...
1. ~~**Enhanced Reactions UI**~~: ✅ Resolved (milestone 90 + recent gChat UI parity). `REACTION` packets, signed/gossiped reaction toggling, and the soft-block/hard-block Content Health overlay are implemented. The UI has been updated to match gChat's standard, using a single-tap trigger to open the reaction picker, and displaying active reactions as inline, tappable pill counters (e.g. `❤️ 3`). Remaining: extending reactions to Direct Messages (`CHAT_REACTION` — see gap analysis doc).
