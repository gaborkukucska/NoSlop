...
**17. Video over Tor still has no exit affinity.** (RESOLVED — 2026-09-06 session)
Resolved via native SOCKS5 stream authentication isolation (`TorSocksSocketFactory` and `TorSocksSocket` implementing RFC 1928 / RFC 1929). In `HttpClientProvider.kt`, each video stream is assigned a unique SOCKS username (`yt_${videoId}_${nonce}`). Tor's `IsolateSOCKSAuth` automatically assigns each username to a dedicated circuit. Both stream resolution in `YouTubeInternalClient` and media chunk fetching in `ExoVideoPlayer`/`PreloadManager` connect using this identical credentials-isolated client, guaranteeing that `googlevideo.com`'s IP lock is satisfied without leaking HTTP headers. If an exit IP is gated with `LOGIN_REQUIRED`, `YouTubeInternalClient` increments `streamNonce` to rotate to a fresh exit node locally without invoking destructive process-wide `SIGNAL NEWNYM` rotations. Micro-seek resume poisoning (<8s) was eliminated and Tor SOCKS connect timeouts were tuned to 35s.
