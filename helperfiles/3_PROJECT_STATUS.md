...
- **2025-12-19** (Session 9):
  - **Admin AI Workflow Orchestration & Chat Fixes - COMPLETE**:
    ...
    - **Implementation**:
      - **Implemented Workflow Generator**: Created `backend/workflow_generator.py` to generate valid ComfyUI JSON from prompts using Ollama.
      - **Workflow Saving**: Workflows are automatically saved to `settings.comfyui_workflows_dir` (e.g., `/mnt/noslop/comfyui/workflows/`) for worker access.
      - **Fixed Chat Truncation**: Increased `num_predict` in `backend/admin_ai.py` (via `backend/config.py`) to allow long, detailed responses.
      - **Prompts**: Added JSON structure examples to `backend/prompts.yaml`.
    - **Files Modified**: `backend/admin_ai.py`, `backend/config.py`, `backend/prompts.yaml`, `backend/workflow_generator.py` (new).
    - **Impact**:
      - ✅ **Robust Chat**: Admin AI no longer cuts off mid-sentence.
      - ✅ **Custom Workflows**: Users can ask "Create a workflow for X" and get a valid, saved JSON file ready for execution.
      - ✅ **End-to-End Execution**: `ImageGenerationWorker` and `ComfyUIClient` now support loading and executing these custom workflows dynamically.
      - **Status**: **Phase 1 Complete**. Workflow generation, saving, and execution are fully operational.
